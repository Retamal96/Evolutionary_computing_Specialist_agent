...
def blend_crossover_ab(population, fitness_population, alpha=0.45, beta=0.55):
    # parent selection
    total_offspring = np.zeros((0,n_vars))
    ...
    while (i + 1) < len(parent_list):
        # choose parents
        p_1 = np.random.randint(0,49)
        p_2 = np.random.randint(0,49)
        fittest_parent = parent_list[p_1]
        not_fittest_parent = parent_list[p_2]

        if fitness_population[p_2] > fitness_population[p_1]:
        ...
        for f in range(0, n_offsprings):
            for gen in range(0, n_vars):
                ...
                offspring[f][gen] = np.random.uniform(not_fittest_parent[gen] - alpha * d,fittest_parent[gen] + d * beta)
...
for i in range(ini_g+1, gens):
    ...
    offspring_list = blend_crossover_ab(population,fitness_population)  # crossover
    ...
    #fitness_population[best] = float(evaluate(np.array([population[best] ]))[0]) # repeats best eval, for stability issues
